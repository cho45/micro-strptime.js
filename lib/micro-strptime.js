function strptime (str, format) {
	...
	var re = new RegExp(format.replace(/%(?:([a-zA-Z%])|('[^']+')|("[^"]+"))/g, function (_, a, b, c) {
		...
		if (!d) throw "Unknown format descripter: " + fd;
		ff.push(d[1]);
		return '(' + d[0] + ')';
	}), 'i');
...
strptime.fd = {
	...
	'm' : [ '[0-9]{2}', function (matched) { this.setUTCMonth(+matched - 1) } ],
	'd' : [ '[0-9]{2}', function (matched) { this.setUTCDate(+matched) } ],
	'H' : [ '[0-9]{2}', function (matched) { this.setUTCHours(+matched) } ],
	'M' : [ '[0-9]{2}', function (matched) { this.setUTCMinutes(+matched) } ],
	'S' : [ '[0-9]{2}', function (matched) { this.setUTCSeconds(+matched) } ],
